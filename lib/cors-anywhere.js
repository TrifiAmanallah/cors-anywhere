...
function proxyRequest(req, res, proxy) {
  ...
  // Start proxying the request
    try {
        try {
            req.headers.host = "localhost:5000" /*proxyOptions.target.host*/;
          console.log("Proxy request with params modified: \n");
          console.log("req.headers: " + JSON.stringify(req.headers));
          console.log("req.url: " + JSON.stringify(req.url));
          console.log("proxyOptions: " + JSON.stringify(proxyOptions));
      } catch (e) {
          console.log("Proxy request log params error: " + e);
      }
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    var proxyBaseUrl = (isRequestedOverHttps ? 'https://' : 'http://') + req.headers.host;
      ...
      corsAnywhere.removeHeaders.forEach(function (header_) {
          let header = header_ ? header_.toLowerCase() : header_;
          delete req.headers[header];
    });

      Object.keys(corsAnywhere.setHeaders).forEach(function (header_) {
          let header = header_ ? header_.toLowerCase() : header_;
